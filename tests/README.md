...
- `tests/engine/`
  ...
  - `tests/engine/test_prompt_assets.py` 与 `tests/engine/test_prompt_composer.py` 共同守住 scene manifest 的当前默认模型边界：推理/交互类 scene 默认应对齐 `mimo-v2-plan-thinking`，写作类 scene 默认应对齐 `mimo-v2-plan`；若测试仍断言旧 `pro` 默认，会把真实 prompt 配置漂移误报成回归
